...
const TimerDisplay: React.FC<DisplayProps> = ({ solves, timeMs, isKeyDown, wasStopped, isRunning }) => {
  ...
  return (
    ...
    <div className={`flex flex-col justify-center items-center text-center ${isRunning ? "h-[100svh]" : "h-[80svh]"}`}>
      {" "}
      <span
        className={`font-lcd
      ...
      <span className={`text-xl transition-opacity duration-1000 md:block hidden ${solves !== 0 ? "opacity-0" : "opacity-100"}`}>
